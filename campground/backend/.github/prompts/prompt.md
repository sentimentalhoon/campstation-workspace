...
You are a senior full‑stack engineer and architect. Produce a complete, ready‑to‑run example project and clear developer instructions for a web application with a Spring Boot 3.5.6 backend (Java 21) and a Next.js frontend (latest version). Deliverables must be minimal but production‑sensible, secure by default, and CI/CD friendly.
...
• 	Frontend: Next.js (latest version), TypeScript. Use App Router, Server Components, Client Components where appropriate, and fetch for API calls. Implement authentication with JWT, storing access token in memory and refresh token in HttpOnly cookie.
...
• 	Testing: Comprehensive unit tests for backend services and controllers (JUnit 5, Mockito), integration tests for at least one endpoint using Testcontainers with PostgreSQL. For frontend, unit tests for components, hooks, and API functions using Jest and React Testing Library, integration tests for pages and authentication flows. Ensure tests are runnable and cover critical paths.
...
• 	Commenting: Use modern and sophisticated commenting practices for backend (JavaDoc with clear descriptions of classes, methods, parameters, and return values) and frontend (JSDoc for TypeScript functions and components, explaining logic and props). Avoid redundant comments; focus on why and how for complex logic.
...
• 	frontend package.json, next.config.js, tsconfig.json, app/layout.tsx, app/page.tsx, app/login/page.tsx, components/ProtectedRoute.tsx, lib/api.ts, __tests__/components/ProtectedRoute.test.tsx, __tests__/lib/api.test.ts.
...
• 	When major modifications are completed, push backend and frontend changes to GitHub. Ensure feat commits are written in Korean.
If any assumption is needed, assume frontend runs on http://localhost:3000 and backend on http://localhost:8080 unless overridden. Use PostgreSQL 15+ for production.
Framing examples
...
• 	Provide one complete Next.js login flow that stores access token in memory and refresh token in HttpOnly cookie, showing how to call protected API and handle 401 -> refresh flow using Server Actions or API routes.
